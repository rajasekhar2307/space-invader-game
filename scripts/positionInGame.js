class InGamePosition {
	...
	shoot() {
		if (this.lastBulletTime == null || (new Date().getTime() - this.lastBulletTime) > (this.settings.bulletMaxFrequency)) {
			...
			play.sounds.playSound('bullet_shot');
		}
	}
	entry(play) {
		this.horizontalMoving = 1;
	...
	draw(play) {
		...
		//draw bullets
		ctx.fillStyle = '#fff622';
		for (let i = 0; i < this.bullets.length; i++) {
			let bullet = this.bullets[i];
			ctx.fillRect(bullet.x - 1, bullet.y - 6, 4, 6);
		}
		//draw UFOS
		for (let i = 0; i < this.ufos.length; i++) {
			let ufo = this.ufos[i];
		...
		ctx.fillStyle = "#fff";
		ctx.textAlign = "left";
		ctx.fillText("'M' to Mute", play.playBoundaries.left, play.playBoundaries.bottom + 70);
    ...
    */
		ctx.fillStyle = "#fff";
		ctx.textAlign = "right";
		ctx.fillText("'P' to Pause", play.playBoundaries.right, play.playBoundaries.bottom + 70);
		//showing score and level
		ctx.fillStyle = "#fdff29";
